...
@section('content')
    ...
    <script>
        ...
        jQuery( document ).ready(function( $ ) {
            ...
            $('#personas_id').select2({
                ...
                allowClear: true,
                dropdownParent: $('#maquinarias-modal'),
                theme: 'bootstrap-5',
                width: '100%',
                ...
                jQuery('#maquinarias-modal').on('hidden.bs.modal', function (event) {
                    limpiarSelect2();
                });
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('Gestión de Maquinarias Pesadas') }}</h1>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
    ...
    <div class="content">
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="row">
                    <div class="col-lg-12 margin-tb"  style="margin-left: 1%;">
                        <div class="">
                            <a class="btn btn-primary" onClick="add()" href="javascript:void(0)"> Nueva Maquinaria Pesada</a>
                ...
                @endif
                <div class="card-body">
                    <table class="table table-bordered table-hover" id="maquinarias" style="width:100%;text-align: center;">
            ...
            <div class="modal fade" id="maquinarias-modal" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Maquinarias</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body container">
                            <form action="javascript:void(0)" id="MaquinariasForm" name="MaquinariasForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                            @csrf    
                            <input type="hidden" name="id" id="id">
                                <div class="form-group">
                                    <label for="placa" class="col-sm-2 control-label">Identificación</label>
                                        <div class="col-sm-12">
                                        <input type="text" class="form-control" id="placa" name="placa" placeholder="Ingresa Identificación" required>
                                    </div>
                                </div>  
                                <div class="form-group">
            ...
            function editFunc(id){
                $.ajax({
                    type:"POST",
                    url: "{{ url('maquinarias/edit') }}",
                    data: { id: id },
                    dataType: 'json',
                    success: function(res){
                        $('#maquinariasModal').html("Editar Maquinarias");
                        $('#maquinarias-modal').modal('show');
                        $('#id').val(res.id);
                        $('#placa').val(res.placa);
            ...
            function deleteFunc(id){
                if (confirm("¿Borrar Registro?\nRecuerde que para eliminar una maquinaria este no debe tener ningún mantenimiento asignado.") == true) {
                    var id = id;
                    // ajax
                    $.ajax({
                        type:"POST",
                        url: "{{ url('maquinarias/delete') }}",
                        data: { id: id },
                        dataType: 'json',
                        success: function(res){
                            var tableex = new DataTable('#maquinarias');
                            tableex.ajax.reload();
                            tableex.draw(false);
                        }
                    });
            ...
            jQuery('#MaquinariasForm').submit(function(e) {
                e.preventDefault();
                var formData = new FormData(this);
                $.ajax({
                    type:'POST',
                    url: "{{ url('maquinarias/store')}}",
                    data: formData,
                    cache:false,
                    contentType: false,
                    processData: false,
                    ...
                    success: (data) => {
                        $("#maquinarias-modal").modal('hide');
                        /*var onTable = $('#maquinarias').dataTable()._fnAjaxUpdate();
